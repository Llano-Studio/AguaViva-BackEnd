  ...
  implements OnModuleInit
{
  ...
  private toSubscriptionPlanResponseDto(
    plan: SubscriptionPlanPrisma & {
      subscription_plan_product?: (SubscriptionPlanProductPrisma & {
    ...
    return {
      ...
      default_deliveries_per_cycle:
        (plan as any).default_deliveries_per_cycle || 1,
      is_active:
  ...
  async update(
    ...
    try {
      ...
      if (
        ...
        throw new ConflictException(
          `Error de conflicto al actualizar el ${this.entityName.toLowerCase()}. Verifique los datos.`,
        );
      }
      handlePrismaError(error, this.entityName);
      throw new InternalServerErrorException(
  ...
  async addProductToPlan(
    planId: number,
    addProductToPlanDto: AddProductToPlanDto,
  ): Promise<SubscriptionPlanResponseDto> {
  ...
  async updateProductInPlan(
    planId: number,
    productId: number,
    updateProductInPlanDto: UpdateProductInPlanDto,
  ): Promise<SubscriptionPlanResponseDto> {
    await this.findOne(planId);
    const product = await this.product.findUnique({
    ...
    try {
      ...
      handlePrismaError(
        error,
        `producto en el ${this.entityName.toLowerCase()}`,
      );
      throw new InternalServerErrorException(
        'Error no manejado después de handlePrismaError',
      );
  ...
  async adjustProductQuantitiesInPlan(
    planId: number,
    dto: AdjustPlanProductQuantitiesDto,
  ): Promise<SubscriptionPlanResponseDto> {
    await this.findOne(planId);
    ...
    for (const adj of productAdjustments) {
      ...
      if (adj.quantity < 0) {
        throw new BadRequestException(
          `La cantidad para el producto ID ${adj.product_id} no puede ser negativa.`,
        );
      }
    }
  ...
  async adjustAllPlansPrice(
    dto: AdjustAllPlansPriceDto,
    ...
    if (
      ...
      throw new BadRequestException(
        "Debe proporcionar un 'percentage' o un 'fixedAmount'.",
      );
    }
    if (
      ...
      throw new BadRequestException(
        "No puede proporcionar 'percentage' y 'fixedAmount' simultáneamente.",
      );
    }
    ...
    for (const plan of plansToUpdate) {
      ...
      if (newPriceDecimal !== null && newPriceDecimal.isNegative()) {
        console.warn(
          `El ajuste para el plan ${plan.name} (ID: ${plan.subscription_plan_id}) resultaría en un precio negativo (${newPriceDecimal?.toFixed(2)}). Se omitirá la actualización de este plan.`,
        );
        continue;
      }
      ...
      if (newPriceDecimal !== null) {
        try {
          ...
          console.error(
            `Error al actualizar el precio del ${this.entityName.toLowerCase()} ${plan.name} (ID: ${plan.subscription_plan_id}):`,
            error,
          );
