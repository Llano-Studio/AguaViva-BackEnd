...
import { CreateDeliveryEvidenceDto, DeliveryEvidenceResponseDto, EvidenceType } from '../dto/delivery-evidence.dto';
...
export class DeliveryEvidenceService extends PrismaClient {
  ...
  private readonly uploadsPath = path.join(process.cwd(), 'public', 'uploads', 'evidence');
   ...
   */
  async saveEvidence(createDto: CreateDeliveryEvidenceDto): Promise<DeliveryEvidenceResponseDto> {
    const { route_sheet_detail_id, evidence_type, evidence_data, created_by } = createDto;
    ...
    try {
      ...
      if (!routeDetail) {
        throw new NotFoundException(`El detalle de hoja de ruta con ID ${route_sheet_detail_id} no existe`);
      }
      
      // 2. Validar y procesar los datos de la evidencia
      if (!evidence_data) {
        throw new BadRequestException('Los datos de evidencia son obligatorios');
      }
   ...
   */
  async getEvidencesByDetail(route_sheet_detail_id: number): Promise<DeliveryEvidenceResponseDto[]> {
  ...
  private async saveEvidenceFile(
    ...
    try {
      ...
      throw new InternalServerErrorException('Error al guardar archivo de evidencia');
    }
  }
