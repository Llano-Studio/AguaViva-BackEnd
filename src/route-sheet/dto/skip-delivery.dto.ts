...
export class SkipDeliveryDto {
  ...
  @IsNotEmpty()
  @IsEnum(SkipDeliveryReason, {
    message: `El motivo debe ser uno de los valores permitidos: ${Object.values(SkipDeliveryReason).join(', ')}`,
  })
  ...
  @ApiPropertyOptional({
    description:
      'Foto de evidencia en formato base64 Data URI (ej. data:image/jpeg;base64,...)',
  ...
  @Matches(/^data:image\/(jpeg|png|gif|bmp);base64,([A-Za-z0-9+/]+={0,2})$/, {
    message:
      'photo_data_uri debe ser un Data URI de imagen base64 válido (jpeg, png, gif, bmp).',
  })
