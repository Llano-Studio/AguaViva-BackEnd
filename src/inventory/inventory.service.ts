...
export class InventoryService extends PrismaClient implements OnModuleInit {
  ...
  async getMovementTypeIdByCode(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Error al obtener ID para ${this.entityNameMovementType.toLowerCase()} con código ${code}.`,
      );
    }
  }
  ...
  async getProductStock(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Error calculando stock para ${this.entityNameProduct.toLowerCase()} ID ${productId}.`,
      );
    }
  }
  ...
  async createStockMovement(
    ...
    const operations = async (
      ...
      if (isEntry) {
        if (!destination_warehouse_id) {
          throw new BadRequestException(
            `Para un movimiento de entrada (${movementType.description}), se requiere destination_warehouse_id.`,
          );
        }
        ...
        if (!source_warehouse_id) {
          throw new BadRequestException(
            `Para un movimiento de salida (${movementType.description}), se requiere source_warehouse_id.`,
          );
        }
        ...
        if (!source_warehouse_id || !destination_warehouse_id) {
          throw new BadRequestException(
            'Para una transferencia, se requieren source_warehouse_id y destination_warehouse_id.',
          );
        }
        if (source_warehouse_id === destination_warehouse_id) {
          throw new BadRequestException(
            'El almacén de origen y destino no pueden ser el mismo para una transferencia.',
          );
        }
      } else {
        if (!source_warehouse_id && !destination_warehouse_id) {
          console.warn(
            `${this.entityNameStockMovement}: Tipo de movimiento '${movementType.code}' no es entrada/salida/transferencia clara y no se especificaron almacenes. El inventario no se actualizará.`,
          );
        }
      }
      ...
      try {
        ...
        throw new InternalServerErrorException(
          `Error creando ${this.entityNameStockMovement.toLowerCase()}.`,
        );
      }
      ...
      try {
        if (
          ...
          (isExit || this.transferMovementTypeCodes.includes(movementTypeCode))
        ) {
        ...
        if (
          ...
          (isEntry || this.transferMovementTypeCodes.includes(movementTypeCode))
        ) {
        ...
        handlePrismaError(error, this.entityNameInventory);
        throw new InternalServerErrorException(
          `Error actualizando ${this.entityNameInventory.toLowerCase()}.`,
        );
      }
    ...
    try {
      ...
      handlePrismaError(error, this.entityNameStockMovement);
      throw new InternalServerErrorException(
        `Error en la transacción al crear ${this.entityNameStockMovement.toLowerCase()}.`,
      );
    }
  }
  ...
  async getFullStockWithDetails(
    ...
    const quantityWhere: Prisma.IntFilter = {};
    ...
    const orderBy = parseSortByString(sortBy, [
    ...
    try {
      ...
      handlePrismaError(error, this.entityNameInventory);
      throw new InternalServerErrorException(
        `Error obteniendo el stock detallado del ${this.entityNameInventory.toLowerCase()}.`,
      );
    }
  }
  ...
  async getStockInWarehouse(
    ...
    try {
      ...
      if (!inventoryItem) {
        throw new NotFoundException(
          `${this.entityNameInventory} no encontrado para ${this.entityNameProduct.toLowerCase()} ID ${productId} en ${this.entityNameWarehouse.toLowerCase()} ID ${warehouseId}.`,
        );
      }
  ...
  async createInitialInventory(
    ...
    const operations = async (
      ...
      try {
        ...
        handlePrismaError(error, this.entityNameInventory);
        throw new InternalServerErrorException(
          `Error creando ${this.entityNameInventory.toLowerCase()} inicial.`,
        );
      }
      ...
      if (quantity > 0) {
        try {
          ...
          console.warn(
            `No se pudo crear movimiento de stock para inventario inicial: ${error.message}`,
          );
        }
      }
    ...
    try {
      ...
      handlePrismaError(error, this.entityNameInventory);
      throw new InternalServerErrorException(
        `Error en la transacción al crear ${this.entityNameInventory.toLowerCase()} inicial.`,
      );
    }
  }
}
