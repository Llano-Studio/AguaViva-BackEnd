...
import { AutomatedCollectionService, CollectionOrderSummaryDto } from '../services/automated-collection.service';
...
export class AutomatedCollectionController {
  ...
  @ApiOperation({ 
    ...
    description: 'Ejecuta la generación de pedidos de cobranza para una fecha específica. Si la fecha cae en domingo, se ajusta automáticamente al sábado anterior.'
  })
  ...
  async generateCollectionOrders(@Body() dto: GenerateCollectionOrdersDto) {
    try {
      ...
      const ordersCreated = results.filter(r => r.order_created && r.notes?.includes('Nuevo pedido')).length;
      const ordersUpdated = results.filter(r => r.order_created && r.notes?.includes('actualizado')).length;
  ...
  @ApiOperation({ 
    ...
    description: 'Lista los ciclos de suscripción que vencen en los próximos días y requieren generación de pedidos de cobranza.'
  })
  @ApiQuery({ 
    name: 'days', 
    required: false, 
  ...
  async getUpcomingCollections(@Query('days') days?: number) {
    try {
      ...
      const upcomingCollections = await this.automatedCollectionService.getUpcomingCollections(periodDays);
      
      const totalAmount = upcomingCollections.reduce((sum, collection) => sum + collection.pending_balance, 0);
  ...
  @ApiOperation({ 
    ...
    description: 'Ejecuta inmediatamente el proceso automático de generación de pedidos de cobranza para la fecha actual.'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async runTodayGeneration() {
    try {
      const results = await this.automatedCollectionService.generateCollectionOrders();
  ...
  @ApiOperation({ 
    ...
    description: 'Obtiene estadísticas sobre el proceso de generación automática de pedidos de cobranza.'
  })
  @ApiQuery({ 
    name: 'days', 
    required: false, 
  ...
  async getCollectionStats(@Query('days') days?: number) {
    try {
      ...
      const cyclesDue = await this.automatedCollectionService['getCyclesDueForCollection'](today);
      
      // Obtener próximas cobranzas para estadísticas
      const upcomingThisWeek = await this.automatedCollectionService.getUpcomingCollections(7);
      const upcomingNextWeek = await this.automatedCollectionService.getUpcomingCollections(14);
      
      const totalPendingAmount = cyclesDue.reduce((sum, cycle) => sum + Number(cycle.pending_balance), 0);
      const overdueCycles = cyclesDue.filter(cycle => cycle.payment_due_date && cycle.payment_due_date < today);
      const overdueAmount = overdueCycles.reduce((sum, cycle) => sum + Number(cycle.pending_balance), 0);
