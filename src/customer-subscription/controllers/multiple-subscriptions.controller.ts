...
import { MultipleSubscriptionsService, MultipleSubscriptionSummaryDto, ActiveCycleSummaryDto } from '../services/multiple-subscriptions.service';
...
export class MultipleSubscriptionsController {
  ...
  @ApiOperation({
    ...
    description: 'Obtiene todas las suscripciones activas de un cliente con información consolidada de pagos y créditos'
  })
  ...
  async getCustomerSubscriptionsSummary(
    ...
    return this.multipleSubscriptionsService.getCustomerSubscriptionsSummary(customerId);
  }
  ...
  @ApiOperation({
    ...
    description: 'Lista todos los ciclos de suscripción activos de un cliente con información detallada de pagos'
  })
  @ApiParam({ name: 'customerId', description: 'ID del cliente', type: Number })
  @ApiResponse({
    status: 200,
  ...
  async getCustomerActiveCycles(
    ...
    return this.multipleSubscriptionsService.getCustomerActiveCycles(customerId);
  }
  ...
  @ApiOperation({
    ...
    description: 'Proporciona estadísticas consolidadas sobre clientes con múltiples suscripciones y distribución de estados de pago'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Consolida información de deudas y créditos de todas las suscripciones activas del cliente'
  })
  @ApiParam({ name: 'customerId', description: 'ID del cliente', type: Number })
  @ApiResponse({
    status: 200,
  ...
  async consolidateCustomerPayments(
    ...
    return this.multipleSubscriptionsService.consolidateCustomerPayments(customerId);
  }
  ...
  @ApiOperation({
    ...
    description: 'Aplica automáticamente los créditos acumulados de todas las suscripciones a las deudas pendientes del cliente'
  })
  @ApiParam({ name: 'customerId', description: 'ID del cliente', type: Number })
  @ApiResponse({
    status: 200,
  ...
  async applyCreditsToCustomerDebts(
    ...
    const summary = await this.multipleSubscriptionsService.getCustomerSubscriptionsSummary(customerId);   
    
    // Aplicar créditos para cada suscripción
    for (const subscription of summary.active_subscriptions) {
      await this.cyclePaymentsService.applyCreditsToOutstandingDebt(subscription.subscription_id);
    }
    
    // Obtener el estado actualizado después de aplicar créditos
    const updatedConsolidation = await this.multipleSubscriptionsService.consolidateCustomerPayments(customerId);
    
    return {
      message: 'Créditos aplicados exitosamente a todas las suscripciones del cliente',
  ...
  @ApiOperation({
    ...
    description: 'Obtiene una lista de todos los clientes que tienen más de una suscripción activa'
  })
  @ApiResponse({
    status: 200,
