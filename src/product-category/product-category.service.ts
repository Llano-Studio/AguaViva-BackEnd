...
import { Prisma, PrismaClient, product_category as ProductCategoryPrisma } from '@prisma/client';
...
export class ProductCategoryService extends PrismaClient implements OnModuleInit {
  ...
  async createProductCategory(dto: CreateProductCategoryDto): Promise<ProductCategoryPrisma> {
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        throw new ConflictException(`La ${this.entityName.toLowerCase()} con el nombre '${dto.name}' ya existe.`);
      }
  ...
  async updateProductCategoryById(id: number, dto: UpdateProductCategoryDto): Promise<ProductCategoryPrisma> {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
         throw new ConflictException(`El nombre de ${this.entityName.toLowerCase()} '${dto.name}' ya está en uso.`);
      }
      handlePrismaError(error, this.entityName);
      throw new InternalServerErrorException('Error no manejado después de handlePrismaError');
    }
  ...
  async deleteProductCategoryById(id: number): Promise<{ message: string; deleted: boolean }> {
    await this.getProductCategoryById(id);
    try {
