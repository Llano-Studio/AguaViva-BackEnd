...
export class FormDataPreserveInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (request.headers['content-type']?.includes('multipart/form-data')) {
      console.log('🔍 DEBUG - FormDataPreserveInterceptor - Request body antes:', request.body);
      ...
      if (request.body) {
        ...
        booleanFields.forEach(field => {
          if (request.body[field] !== undefined) {
            ...
            console.log(`🔍 DEBUG - FormDataPreserveInterceptor - Preservando ${field}: ${originalValue} -> ${request.body[field]} (${typeof request.body[field]})`);
          }
        });
        
        console.log('🔍 DEBUG - FormDataPreserveInterceptor - Request body después:', request.body);
      }
    }
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        console.log('🔍 DEBUG - FormDataPreserveInterceptor - Request body al final:', request.body);
      })
    );
  }
}