...
export class PdfGeneratorService {
  // Normaliza hex con alpha (#RRGGBBAA o #RGBA) a #RRGGBB (PDFKit no acepta alpha en hex)
  private normalizeHexColor(hex?: string): string | undefined {
    ...
    // #RGBA -> #RRGGBB
    const m4 = /^#([0-9a-fA-F]{4})$/.exec(hex);
    if (m4) {
      const r = m4[1][0];
      const g = m4[1][1];
      const b = m4[1][2];
      // ignoramos el canal alpha m4[1][3]
      return `#${r}${r}${g}${g}${b}${b}`;
    }
    return hex;
  }

  // Convierte color hex a RGB array para PDFKit
  ...
  // Obtiene el color en formato que PDFKit acepta (RGB array o hex válido)
  private getPdfColor(colorHex?: string, fallback: string = '#000000'): string | [number, number, number] {
    const normalizedHex = this.normalizeHexColor(colorHex) || fallback;
    // Intentar convertir a RGB para mejor compatibilidad
    const rgb = this.hexToRgb(normalizedHex);
    return rgb || normalizedHex;
  }
  /**
  ...
  private async generateRouteSheetContent(
    ...
    // Configuración de colores (PDFKit no soporta #RRGGBBAA)
    const primaryColor = this.getPdfColor(customColors.primary, '#181594');
    const secondaryColor = this.getPdfColor(customColors.secondary, '#0047aa');
    const accentColor = this.getPdfColor(customColors.accent, '#f59e0b');
    const lightGray = this.getPdfColor(customColors.lightGray, '#f3f4f6');
    ...
    doc
      .fontSize(28)
      .fillColor('white')
      .text('HOJA DE RUTA', 50, 25, {
        align: 'center',
        width: doc.page.width - 100,
      });
    doc.fontSize(12).text(`#${routeSheet.route_sheet_id}`, 50, 50, {
      align: 'center',
      width: doc.page.width - 100,
    });
  ...
  doc.fillColor('#1fd8cf').fontSize(14).font('Helvetica-Bold');
    ...
    doc.text('PEDIDOS A ENTREGAR', 50, currentY, {
      align: 'center',
      width: doc.page.width - 100,
    });
    currentY += 30;
    ...
    for (let i = 0; i < routeSheet.details.length; i++) {
      ...
      colX = startX + 5;
      doc.text(detail.order.order_id.toString(), colX, currentY + 8, {
      ...
      colX += colWidths[2];
      doc.text(detail.order.customer.phone, colX, currentY + 8, {
      ...
      doc.text(translatedStatus, colX, currentY + 8, {
    ...
    if (includeSignatureField) {
      ...
      doc.fillColor(primaryColor).fontSize(18).font('Helvetica-Bold');
      doc.text('CONFIRMACION DE ENTREGAS', 50, currentY, {
        align: 'center',
        width: doc.page.width - 100,
      });
      currentY += 50;
      ...
      // Línea de firma
      doc
        .moveTo(60, signatureY + 40)
        .lineTo(280, signatureY + 40)
        .stroke();
      doc
        .moveTo(60, signatureY + 60)
        .lineTo(280, signatureY + 60)
        .stroke();
      doc
        .moveTo(60, signatureY + 80)
        .lineTo(280, signatureY + 80)
        .stroke();
      ...
      doc
        ...
        .lineTo(540, signatureY + 40)
        .stroke();
      doc
        .moveTo(320, signatureY + 60)
        .lineTo(540, signatureY + 60)
        .stroke();
      doc
        .moveTo(320, signatureY + 80)
        .lineTo(540, signatureY + 80)
        .stroke();
    }
    ...
    doc.text(`Total de pedidos: ${routeSheet.details.length}`, {
      align: 'right',
    });
  }
