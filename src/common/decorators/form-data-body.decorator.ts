...
export const FormDataBody = createParamDecorator(
  async (data: any, ctx: ExecutionContext) => {
    ...
    if (request.headers['content-type']?.includes('multipart/form-data')) {
      ...
      const booleanFields = ['is_returnable', 'isActive', 'is_active', 'owns_returnable_containers'];
      ...
      if (dtoName.includes('Product')) {
        allowedFields = ['category_id', 'description', 'volume_liters', 'price', 'is_returnable', 'serial_number', 'notes', 'total_stock', 'productImage'];
      } else if (dtoName.includes('User') || dtoName.includes('Register')) {
        allowedFields = ['email', 'password', 'name', 'role', 'isActive', 'notes', 'profileImage'];
      } else if (dtoName.includes('Person')) {
        allowedFields = ['name', 'address', 'phone', 'tax_id', 'email', 'birth_date', 'type', 'locality_id', 'zone_id', 'owns_returnable_containers', 'is_active', 'notes'];
      } else {
      ...
      Object.keys(preservedBody).forEach(key => {
        if (allowedFields.includes(key)) {
          ...
          console.log(`🔍 DEBUG - FormDataBody - Filtrando campo no permitido: ${key}`);
        }
      });
      ...
      booleanFields.forEach(field => {
        if (filteredBody[field] !== undefined) {
          ...
          console.log(`🔍 DEBUG - FormDataBody - Preservando ${field} como string:`, filteredBody[field]);
        }
      });
      ...
      if (dtoClass) {
        ...
        if (errors.length > 0) {
          console.log('🔍 DEBUG - FormDataBody - Errores de validación:', errors);
