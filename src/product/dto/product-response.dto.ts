...
export class WarehouseResponseDto {
  ...
  @ApiProperty({
    ...
    description: 'Localidad del almacén',
    required: false,
  })
...
export class ProductInventoryResponseDto {
  ...
  @ApiProperty({
    example: 50,
    description: 'Cantidad disponible en este almacén',
  })
  quantity: number;
...
export class ProductResponseDto extends OmitType(CreateProductDto, [
  'category_id',
] as const) {
  ...
  @ApiProperty({
    example: 100,
    description: 'Stock total disponible del producto en todos los almacenes',
  })
  total_stock: number;
  ...
  constructor(
    partial: Partial<ProductResponseDto> & {
    ...
    if (
      typeof partial.volume_liters === 'object' &&
      partial.volume_liters !== null
    ) {
    ...
    this.serial_number =
      partial.serial_number === null ? undefined : partial.serial_number;
